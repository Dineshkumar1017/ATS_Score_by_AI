...
def _detect_file_type(file_data: bytes, filename: str) -> tuple[str | None, str | None]:
    if magic is not None:
        try:
            ...
            log_warning(
                f"libmagic MIME detection unavailable, falling back to extension: {exc}",
                context="resume_parser",
            )

    extension = Path(filename).suffix.lower()
    if extension == ".pdf" and file_data.startswith(b"%PDF"):
        return "pdf", "application/pdf"
    if extension == ".docx" and file_data.startswith(b"PK"):
        return (
            "docx",
